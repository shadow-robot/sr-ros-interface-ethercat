...
//!  <tr> <td>MOTOR_DATA_GIT_REVISION</td>            <td colspan=2>Server revision</td>    <td colspan=2>This revision (top bit = modified)</td> </tr>
...
typedef enum
{
    ...
    MOTOR_SLOW_DATA_GIT_REVISION         = 0x0001,              //!< The revision of the code
    MOTOR_SLOW_DATA_GIT_SERVER_REVISION  = 0x0002,              //!< The revision of the code on the Git server at build time.
                                                                //!  Should we have done an Update before building?
    MOTOR_SLOW_DATA_GIT_MODIFIED         = 0x0003,              //!< Did the local code have any uncomitted modifications at build time?
...
#define PALM_0240_EDC_SERIOUS_ERROR_FLAGS            (  MOTOR_FLAG_BITS_NO_DEMAND_SEEN         \
                                                      | MOTOR_FLAG_BITS_A3950_NFAULT           \
                                                      | MOTOR_FLAG_BITS_EEPROM_CONFIG_BAD_CRC  \
                                                      | MOTOR_FLAG_BITS_OVER_TEMP    )


#define PALM_0240_EDC_NO_DEMAND_TIMEOUT_MS    20                                                              //!< If a motor doesn't see any Torque or PWM demand values,
    ...
    static const char* palm_0240_edc_error_flag_names[16] = { "Current choke bit 6",                          // 0x0001
...
#define SENSORS_NUM_0240  ((int)36)                             //!< The number of sensors in the robot.
                                                                //!  This needs to be a #define because it's used to dimension an array.

#define JOINTS_NUM_0240   ((int)28)                             //!< The number of joints in the hand
                                                                //!  This needs to be a #define for symmetry with SENSORS_NUM


//#if (int)IGNORE > SENSORS_NUM
//    #error Not enough sensors[] in ETHERCAT_DATA_STRUCTURE_0240_PALM_EDC_STATUS
//#endif
...
typedef struct
{
	...
	int16u					    sensors[SENSORS_NUM_0240+1];
