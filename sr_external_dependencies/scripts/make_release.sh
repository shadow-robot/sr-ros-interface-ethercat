...
echo "protocol_motor:"${VERS_0220} "; protocol_muscle:"${VERS_0320_MUSCLE} "; protocol_ubi_ts:"${VERS_0230_TS} "; bootloader:"${VERS_BOOTLD}" ; firmware:"${VERS_FW}" ; git_shadow:"${VERS_GIT_SHADOW} "; git_ethercat:"${VERS_GIT_ETHERCAT} > tested_version.yaml

git tag --force "protocol_motor_"${VERS_0220}"__protocol_muscle_"${VERS_0320_MUSCLE}"__protocol_ubi_ts_"${VERS_0230_TS}"__bootloader_"${VERS_BOOTLD}"__firmware_"${VERS_FW}"__git_shadow_"${VERS_GIT_SHADOW}"__git_ethercat_"${VERS_GIT_ETHERCAT}
