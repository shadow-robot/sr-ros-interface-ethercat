...
inline void write_status_data_To_ET1200(void)
{
  ...
  write_DMA_ET1200_register_N(ETHERCAT_STATUS_DATA_ADDRESS, ETHERCAT_STATUS_DATA_SIZE,
...
void Service_EtherCAT_Packet(void)
{
  ...
  switch (etherCAT_command_data.EDC_command)
  {
    ...
    case EDC_COMMAND_CAN_DIRECT_MODE:                                                   // CAN Bridge packet
      ...
      {
        read_ET1200_register_N(ETHERCAT_CAN_BRIDGE_DATA_COMMAND_ADDRESS, ETHERCAT_CAN_BRIDGE_DATA_SIZE,
      ...
      write_ET1200_register_N(ETHERCAT_CAN_BRIDGE_DATA_STATUS_ADDRESS, ETHERCAT_CAN_BRIDGE_DATA_SIZE,
...
void Received_Muscle_Data(FROM_MUSCLE_DATA_TYPE data_type, int8u muscle_data_packet_number,
                          MUSCLE_DATA_PACKET *muscle_data_packet)
{

  if (muscle_data_packet_number >
      7)                                                                       // A muscle_driver_number > 1 is an error
...
*/                                                                                                        // unpacks the data.
    ...
    etherCAT_status_data.which_muscle_data_arrived |=
            0x00000001 << muscle_data_packet_number;   // Record that a message arrived
