...
namespace tactiles
{
  ...
  void Biotac<StatusType, CommandType>::update(StatusType *status_data)
  {
    ...
    for (unsigned int id_sensor = 0; id_sensor < this->nb_tactiles; ++id_sensor)
    {
      ...
      tactiles_vector->at(id_sensor).pac_buffer_.push_back(static_cast<int>(tactile_data->Pac[0]));
      tactiles_vector->at(id_sensor).pac_buffer_.push_back(static_cast<int>(tactile_data->Pac[1]));
