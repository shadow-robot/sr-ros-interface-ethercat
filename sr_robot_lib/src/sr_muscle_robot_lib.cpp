...
CalibrationMap SrMuscleRobotLib<StatusType, CommandType>::read_pressure_calibration()
{
  ...
  for (int32_t index_cal = 0; index_cal < calib.size(); ++index_cal)
  {
    ...
    pressure_calibration.insert(joint_name, shared_ptr<shadow_robot::JointCalibration>(new shadow_robot::JointCalibration(calib_table_tmp)));
  }
...
template <class StatusType, class CommandType>
void SrMuscleRobotLib<StatusType, CommandType>::read_additional_muscle_data(vector<Joint>::iterator joint_tmp, StatusType* status_data)
{
  if (!joint_tmp->has_actuator)
    return;
...
template <class StatusType, class CommandType>
void SrMuscleRobotLib<StatusType, CommandType>::read_muscle_driver_data(vector<MuscleDriver>::iterator muscle_driver_tmp, StatusType* status_data)
{
...
void SrMuscleRobotLib<StatusType, CommandType>::reinitialize_motors()
{
  ...
  muscle_updater_ = shared_ptr<MuscleUpdater<CommandType> >(new MuscleUpdater<CommandType>(muscle_update_rate_configs_vector, operation_mode::device_update_state::INITIALIZATION));
