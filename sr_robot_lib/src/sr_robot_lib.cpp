...
CalibrationMap SrRobotLib<StatusType, CommandType>::read_joint_calibration()
{
  ...
  for (int32_t index_cal = 0; index_cal < calib.size(); ++index_cal)
  {
    ...
    joint_calibration.insert(joint_name, shared_ptr<shadow_robot::JointCalibration>(new shadow_robot::JointCalibration(calib_table_tmp)));
  }
...
template <class StatusType, class CommandType>
vector<UpdateConfig> SrRobotLib<StatusType, CommandType>::read_update_rate_configs(string base_param, int nb_data_defined, const char* human_readable_data_types[], const int32u data_types[])
{
