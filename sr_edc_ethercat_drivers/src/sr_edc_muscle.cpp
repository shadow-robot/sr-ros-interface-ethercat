 ...
 * If you need to have several commands like in this SrEdcMuscle driver, put the sum of the size, same thing for the status.
 *
 */
void SrEdcMuscle::construct(EtherCAT_SlaveHandler *sh, int &start_address)
{
...
int SrEdcMuscle::initialize(pr2_hardware_interface::HardwareInterface *hw, bool allow_unprogrammed)
{
  ...
  sr_hand_lib = boost::shared_ptr<shadow_robot::SrMotorHandLib<ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_STATUS, ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_COMMAND> >( new shadow_robot::SrMotorHandLib<ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_STATUS, ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_COMMAND>(hw) );
 ...
 *  \endcode
 */
void SrEdcMuscle::packCommand(unsigned char *buffer, bool halt, bool reset)
{
 ...
 *  \code
 *  ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_STATUS *tbuffer = (ETHERCAT_DATA_STRUCTURE_0300_PALM_EDC_STATUS *)(this_buffer + command_size_);
...
void SrEdcMuscle::reinitialize_boards()
{
  //Reinitialize motors information
  sr_hand_lib->reinitialize_motors();
}

void SrEdcMuscle::get_board_id_and_can_bus(int board_id, int *can_bus, unsigned int *board_can_id)
{
  // We're using 2 can busses,
  // if muscle_driver_board id is between 0 and 1, then we're using the can_bus 1
  // else, we're using the can bus 2.
  int muscle_driver_board_id_tmp = board_id;
  if( muscle_driver_board_id_tmp > 1 )
  {
    muscle_driver_board_id_tmp -= 2;
    *can_bus = 2;
  }
  else
  {
  ...
  *board_can_id = muscle_driver_board_id_tmp;
}
